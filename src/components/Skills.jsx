...
const Skills = () => {
  ...
  // Mobile scroll handler using requestAnimationFrame for 60fps performance
  const handleScroll = (e) => {
    ...
    bgRefs.current.forEach((bg, i) => {
      if (bg) gsap.to(bg, { opacity: i === activeIdx ? 1 : 0, duration: 0.3, overwrite: "auto" });
    });
    
    textRefs.current.forEach((txt, i) => {
      if (txt) gsap.to(txt, { opacity: i === activeIdx ? 1 : 0, duration: 0.3, overwrite: "auto" });
    });
  };
  ...
  useGSAP(() => {
    ...
    mm.add("(max-width: 768px)", () => {
      cardsRef.current.forEach((card, i) => {
        if (card) {
          gsap.set(card, { clearProps: "x,y,z,rotation,scale,opacity,zIndex" });
  ...
  return (
    ...
    >
      ...
      {/* 3D Arc / Touch Carousel Container */}
      <div 
